...
$name=$birthday=$email=$password=$postal_code=$address=$phone_number='';
                ...
                <fieldset>
                    ...
                    <form action="signup-output.php" method="post" class="error_check form-horizontal">
                        <div class="form-group">
                            ...
                            <input type="text" id="name" name="name" value="<?= $name;?>" class="form-control" placeholder="鈴木一郎" required aria-required="true" autofocus>
                        </div>
                         <div class="form-group">        
                            <label for="birthday" class="control-label"><span class="required">生年月日（※必須）</span>：（※西暦生年月日・半角数字）</label>
                            <input type="text" id="birthday" name="birthday" value="<?= $birthday;?>" class="form-control col-sm-6" placeholder="1990/01/01" pattern="^(\d{4})/(0[1-9]|1[0-2])/(0[1-9]|[12][0-9]|3[01])$" required aria-required="true">
                        </div>
                        <div class="form-group">        
                            <label for="email" class="control-label"><span class="required">メールアドレス（※必須）</span>：（例：suzuki@xxx.com）</label>
                            <input type="email" id="email" name="email" value="<?= $email;?>" class="form-control col-sm-6" placeholder="suzuki@xxx.com" pattern="^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$" required aria-required="true">
                        </div>
                        <div class="form-group">        
